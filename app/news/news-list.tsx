...
import { ArrowRight, Search, PenLine, Pin } from 'lucide-react';
...
export function NewsList({ initialPosts }: { initialPosts: NewsPost[] | null }) {
  ...
  return <div className={styles.container}>
    <section className={styles.intro}><div><p className={styles.eyebrow}>메이플스토리 소식</p><h1>알아두면 좋은 소식,<br /><span>판매 전에 한 번 더.</span></h1><p className={styles.description}>메이플 이벤트부터 아이템 시세 정보까지.<br />필요한 소식을 찾아보고, 아이템 정리를 준비하세요.</p></div><Link href="/news/write" className={styles.secondary}><PenLine size={17} aria-hidden="true" /> 마크다운 작성</Link></section>
